...
# Data Visualization
* The goal of this lesson is to help you create visualizations that clearly describe one or more variables and to explore patterns across groups or between multiple variables. To succeed, you will learn several common graph types in R, with a focus on ggplot2, a powerful and widely used graphics package within the tidyverse. Because R packages evolve quickly, some parameters may change over time, so it’s important to stay flexible, explore options, and rely on the open-source community for updated guidance.

* This lesson provides a strong starting point rather than an exhaustive list of every visualization available in R. You will learn how to create graphs for a single categorical variable, a single continuous variable, and two variables at once. Throughout the lesson, we emphasize the importance of choosing visuals based on the variable type—categorical or quantitative—and ensuring that your data is coded correctly before you build your chart. You will work with bar charts, density plots, boxplots, scatterplots, and multiple-boxplot comparisons to understand both distributions and relationships between variables.

* As you read, remember that visualizing data is one of the most effective ways to understand what the data is saying—yet visuals can be misleading if the data is handled improperly or if the graph is constructed to force a preferred story. Both base R and ggplot2 can produce many of the same graphs, but ggplot2 provides more flexibility and a layered structure that makes customization easier. Other specialized packages, such as waffle, also exist for specific types of graphics. By the end of the lesson, you should feel comfortable selecting and creating appropriate visualizations for different data types and analytical goals.
